...
import { LLMCheckInputNodeProps, schema } from "./typedef";
import Main from ".";


/**
 * LLM input checking component
 * @param props
 * @returns
 */
export default function LLMCheckInputForm(
    props: Node & { data: LLMCheckInputNodeProps["data"] }
): React.ReactNode {
    const { setNodes } = useReactFlow();
    const { options } = useWorkSpace();

    // handle state
    const formik = useFormik<LLMCheckInputNodeProps["data"]>({
        initialValues: {
            ...schema.getDefault(),
            ...(props.data || {})
        },
