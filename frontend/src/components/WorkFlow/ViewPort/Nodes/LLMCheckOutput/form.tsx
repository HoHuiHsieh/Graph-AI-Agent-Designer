...
import { LLMCheckOutputNodeProps, schema } from "./typedef";
import Main from ".";


/**
 * LLM output checking component
 * @param props
 * @returns
 */
export default function LLMCheckOutputForm(
    props: Node & { data: LLMCheckOutputNodeProps["data"] }
): React.ReactNode {
    const { setNodes } = useReactFlow();
    const { options } = useWorkSpace();

    // handle state
    const formik = useFormik<LLMCheckOutputNodeProps["data"]>({
        initialValues: {
            ...schema.getDefault(),
            ...(props.data || {})
        },
