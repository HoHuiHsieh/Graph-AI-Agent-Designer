...
import { LLMSummarizeNodeProps, schema } from "./typedef";
import Main from ".";


/**
 * LLM summarize component
 * @param props
 * @returns
 */
export default function LLMSummarizeForm(
    props: Node & { data: LLMSummarizeNodeProps["data"] }
): React.ReactNode {
    const { setNodes } = useReactFlow();
    const { options } = useWorkSpace();

    // handle state
    const formik = useFormik<LLMSummarizeNodeProps["data"]>({
        initialValues: {
            ...schema.getDefault(),
            ...(props.data || {})
        },
