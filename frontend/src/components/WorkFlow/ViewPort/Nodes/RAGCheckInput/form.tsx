...
import { Divider, Typography, TextField, Stack, Autocomplete, Tooltip, IconButton, InputAdornment, MenuItem, Box, Tab, Paper, FormControlLabel, Switch } from "@mui/material";
...
import { schema, RAGCheckInputNodeProps, seperators } from "./typedef";
import Main from ".";
...
/**
 * RAG check input component
 * @param props
 * @returns
 */
export default function RAGCheckInputForm(
    props: Node & { data: RAGCheckInputNodeProps["data"] }
): React.ReactNode {
    ...
    // handle state
    const formik = useFormik<RAGCheckInputNodeProps["data"]>({
        initialValues: {
            ...schema.getDefault(),
            ...(ConvRAGData.decode(props.data, schema.getDefault()["documents"]) as RAGCheckInputNodeProps["data"] || {}),
            id: props.id,
        },
    ...
    useEffect(() => {
        return () => {
            setNodes((nodes) => {
                ...
                if (index > -1) {
                    nodes[index].data = ConvRAGData.encode(formik.values, schema.getDefault()["documents"]);
                }
                return nodes;
            });
        }
    ...
    const handleUploadDocument = useCallback(
        (event: React.ChangeEvent<HTMLInputElement>) => {
            ...
            reader.onloadend = async () => {
                let data = formik.values.documents;
                data.push({ text: file, immoral: false });
                formik.setFieldValue("documents", data);
            };
    ...
    const handleBuildDatabase = useCallback<React.MouseEventHandler<HTMLButtonElement>>(
        (event) => {
            setLoading(true);
            let props = ConvRAGData.encode(formik.values, schema.getDefault()["documents"]);
            getSimpleRAG.build({
                id: props.id,
                model: props.model,
                ...
                .then(() => {
                    ...
                    let tmp = schema.getDefault();
                    let props = ConvRAGData.decode({
                        ...formik.values,
                        model: info.model || tmp.model,
                        documents: info.documents.map(e => ({ text: e, immoral: false })) || [],
                        parameters: {
                    ...
                    }, tmp["documents"]) as RAGCheckInputNodeProps["data"];
